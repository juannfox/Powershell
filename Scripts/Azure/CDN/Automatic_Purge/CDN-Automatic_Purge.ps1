...
function Get-ContainerModifiedBlob($StorageAccount_Context,$StorageAccount_Container_Name,$Date_Treshold_Hours,$Debug=$False){
	<#
...
function Get-StorageAccountModifiedBlob($StorageAccount_Context, $StorageAccount_Container_Name, $Date_Treshold_Hours, $Debug = $False){
	<#
    ...
    if ($StorageAccount_Container_Name -in ("",$null,$false,"/","*")){#Mode 1: Wildcard, loop through all containers
        if ($Debug){Write-Warning "Trabajando en modo Wildcard (sobre todos los Containers) y exceoptuando los que empiezan con $."}
            ...
            foreach ($Container in $Containers){
                ...
                if (-not($Container.StartsWith("$"))){
                    ...
                    $CDN_Content_URLPaths_ThisContainer=Get-ContainerModifiedBlob -StorageAccount_Context $StorageAccount_Context -StorageAccount_Container_Name $Container -Date_Treshold_Hours $Date_Treshold_Hours -Debug $Debug
		...
		#Fetches the blob files modified after treshold time within the container.
        $CDN_Content_URLPaths=Get-ContainerModifiedBlob -StorageAccount_Context $StorageAccount_Context -StorageAccount_Container_Name $StorageAccount_Container_Name -Date_Treshold_Hours $Date_Treshold_Hours -Debug $Debug
    }
...
function Main ($StorageAccount,$CDNProfile){
    ...
    $CDN_Content_URLPaths=Get-StorageAccountModifiedBlob -StorageAccount_Context $StorageAccount_Context -StorageAccount_Container_Name $StorageAccount["Container"] -Date_Treshold_Hours $Date_Treshold_Hours -Debug $Debug
